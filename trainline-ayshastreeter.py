...
if selected_operator:
    ...
    if filtered_df.empty:
        ...
        monthly_total = filtered_df.groupby(['year','month'])['sales'].sum().reset_index(name='total_sales')

        monthly_rurality = (
            filtered_df.groupby(['year','month','rurality_nm'])['sales']
            .sum()
            .reset_index(name='rurality_sales')
        )

        monthly_sales = pd.merge(monthly_total, monthly_rurality, on=['year','month'], how='inner')
        monthly_sales['rurality_pct'] = (monthly_sales['rurality_sales'] / monthly_sales['total_sales']) * 100
        ...
        fig_rurality.update_layout(barmode="stack", xaxis=dict(tickmode="linear", tick0=1, dtick=1))
